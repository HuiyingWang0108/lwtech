...
public class TwoMethodsA04 {
	...
	/**
	 * The method that takes two parameters, size and direction,
	 * and returns a string with a pattern of numbers.
	 * 
	 * @param size      It defines the size of pattern and the data type is int.
	 * @param direction It defines the direction of the pattern.
	 * @return
	...
	/**
	 * Analyzes a file of integers and writes statistics to an output file.
	 * 
	 * @param inputFileName  The name of the input file containing integers, one per
	 *                       line.
	 * @param outputFileName The name of the output file to write analysis results.
	 * @throws IOException
	 */
	public static void fileAnalysis(String inputFileName, String outputFileName) throws IOException {
		try (Scanner scanner = new Scanner(new File(inputFileName));
			...
			int numIntegers = 0;// The number of Integers
			int largest = Integer.MIN_VALUE;// initialize a biggest number
			int smallest = Integer.MAX_VALUE;// initialize a smallest number
			int sum = 0;// The sum of all numbers
			String oneVal = "";// To save the only one line value
			boolean hasFistLine = false;// initialize the first line is without value

			while (scanner.hasNextLine()) {
				String line = scanner.nextLine();
				if (!hasFistLine)
					oneVal += line;// assign the fist line to oneVal
				hasFistLine = true;
				try {
					...
					// Skip invalid lines that are not integers.
				}
			}

			if (numIntegers > 1) {// outputFile has the following in it when the total
				// inputFile numbers is bigger than 1 :
				writer.write("Numeric Data File \"" + inputFileName + "\" Analysis\n");
				writer.write("Number of integers: " + numIntegers + "\n");
				writer.write("The sum of all integers in file: " + sum + "\n");
				writer.write("The largest integer in the set: " + largest + "\n");
				writer.write("The smallest integer in the set: " + smallest + "\n");
			} else if (numIntegers == 1) {
				writer.write("Numeric data file \"" + inputFileName + "\" has only one number: " + oneVal
						+ "\n");
			} else {
				writer.write("Numeric data file \"" + inputFileName + "\" is empty\n");
			}
		}
	 ...
	 * Please see those files to make sure your file output format matches the
	 * output format in test files
	 */
